...
\title{Joint Virtual Middleboxes and Application Placement with Bandwidth Guarantees in Multi-tenant Datacenter Networks}
%\begin{comment}
\author{Jian Wang}
...
a solution that combines a tenant request model with an effective placement algorithm for middleboxes and application, 
%provides bandwidth guarantee to both middleboxes and application, 
while conducting a probability model based approach to addressing the middlebox scaling problem. We first specify the model of the tenant's requirement that matches the demands for virtual machines of application and middleboxes, as well as communication traffic. A joint virtual middleboxes and application placement algorithm 
...
is then proposed to offer predictable network performance for each  tenant, and minimize  datacenter bandwidth utilization. Finally, we provide an approach to solving the middlebox scaling problem through reserving additional free virtual machine (VM) resources based on a probablility model. Extensive simulation results based on current large-scale datacenter networks verify that vMAP can accept more tenant requests than existing placement algorithm (e.g., Stratos) while saving  $\sim$10\% core-level bandwidth. The middlebox scaling experiments also show that our middlebox scaling approach can accommodate the scaling request effectively while minimizing the waste  of the reserved virtual machine resources.
...
these two VM partitions. However, the location-unawareness rack selection in Stratos leads to bandwidth waste of core-level links which are usually the bottlenecks for most oversubscription data center networks. The inter-partition communication of the partition pair may travel through several switches %due to improper rack selection 
even the partitions can be placed under the same switch, since Stratos only considers the bandwidth resource apart from the location of the rack pair when selecting racks.  % since the algorithm only considers the VM slots and bandwidth resources when selecting the racks to accomadate the partitions, which leads to a bandwidth waste of core-level links.  
...
The performanc of middlebox is hard to predict in practice. The experience based middlebox selection may cause an inexact estimation of the resource consumption and impact the application performace. %To avoid the situation that the middleboxes obtained by estimation cannot fully handle the application workload, we need to    
...
Therefore, we seek to firstly consider enforcing one placement principle and then try to improve another principle's benefit by carefully violating the enforced principle before. %previously.
...
The total number of instances required by $MB_k$ is indicated by $M_k$\footnote{We assume each instance of $MB_k$ has a maximal traffic processing capacity $c_k$, so the minimum required number of $MB_k$ is defined as 
	...
	$M_{k}={\left\lceil\frac{N \cdot B^{ex}}{c_{k}}\right\rceil}$,
	%\end{equation}
	where $N\cdot B^{ex}$ is the total incoming traffic.}. And $m_k$ denotes the number of instances placed for $MB_k$  in the left subtree. %The number of VMs for the application is $n_{app}$ in the left subtree.%
 Correspondingly, the number of instances for $MB_k$ in the right subtree is $M_k-m_k$. The numbers of APP VMs in the left and right subtrees are denoted by $n_{app}$ and $N-n_{app}$ respectively. %Suppose 
...
\end{aligned}
\end{equation}
where $b_k$ is the required bandwidth\footnote{For each VMN model, %we assume that 
...
More specifically, to reduce the traffic $B_{mb_k,mb_{k+1}}$ and $B_{mb,app}$ crossing the subtrees caused by the mismatched capacities between the upstream and downstream MBs or APPs, we can place the MBs and APPs proportionally in the subtree according to their traffic processing capacity. In addition, %as $n_{app}$ increases from zero to $N$, 
...
We then analyze the change of the total bandwidth requirement before and after the position exchanging to help to select the proper MB to exchange.% in detail.
\begin{figure}
	\centering
...
We further analyze the bandwidth variation of the single link %to obtain enhanced 
...
Since middleboxes are widely used in current networks, it is essential to consider middleboxes when deploying the tenant request with performance guarantees. %embedding predicable virtual networks. 
 %This paper tries to fill the gap. 
%To achieve this goal, 
In this paper, we introduced vMAP, a joint virtual middleboxex and application deployment solution. We first models the tenant's requirement of software middleboxes and applications. By augumenting software middleboxes as VMs in the Tenant Application Graph, a VMs placement algorithm %called EMA, 
...
He currently focuses on the field of resources management in datacenter networks.
\end{window} 
