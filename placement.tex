...
\title{Joint Virtual Middlebox and Application Placement with Bandwidth Guarantees in Multi-tenant Datacenter Networks}
%\begin{comment}
\author{Jian Wang}
\ead{wjian@dislab.nju.edu.cn}
\author{Xuewei Zhang}
\ead{zhangxw@dislab.nju.edu.cn}
\author{Xiaoliang Wang\corref{correspondingauthor}}
\cortext[correspondingauthor]{Corresponding author.}
\ead{waxili@nju.edu.cn}
\author{Sheng Zhang\corref{}}
\ead{sheng@nju.edu.cn}
\author{Sanglu Lu\corref{}}
\ead{sanglu@nju.edu.cn}
\address{National Key Laboratory for Novel Software Technology, Nanjing University, Nanjing, China}
%\end{comment}
\begin{comment}
\author{\IEEEauthorblockN{Jian Wang, Xuewei Zhang, Xiaoliang Wang, Sheng Zhang, Sanglu Lu}
...
%the application workload is varying in practice, and a middlebox scaling approach is then required to be considerd in practice to better handle the workload.

%Inspired by recent works on application-driven bandwidth guarantee \cite{B13cta, cloudmirror},
...
%we consider the middlebox placement strategy to overcome these challenges. 
...
%of the partition pair 
may travel through several switches %due to improper rack selection 
...
%pair when selecting racks to accommodate the partitions. %However, 
...
%Specifically, Stratos adopts a greedy approach to selecting a pair of racks with the highest available bandwidth for each partition pair that has communication.  
...
%Moreover, 
%to make good use of both physical machine resource and network bandwidth resource, 
%we consider dividing middleboxes and application VMs proportionally and putting them closely to each other 
%in the servers or racks under the same subtree 
%to
...
%the mismatched assignment of traffic processing capacity. 
...
It is notable that the performance of middlebox is hard to predict in practice. The experience based middlebox selection may cause under-/over-estimation of the resource consumption in the shared environment and affect the performance of deployed applications. %To avoid the situation that the middleboxes obtained by estimation cannot fully handle the application workload, we need to    
%The application workload is variable in practice, which necessitates scaling middleboxes 
%Further, %the application performance will be affected if the estimated number of middleboxes 
%in order to 
Hence, 
%to provide predictable application performance,
it is necessary to conduct a middlebox scaling approach %request 
%The middlebox scaling request is  
when the capacity of allocated middleboxes cannot handle the practical workload well. 
%To address the middlebox scaling problem, 
%In contrast to dynamically allocating slot resources when the application is running, %which is inefficient and may cause improper placement, 
...
%The fast and localized deployment 
can improve the application performance and reduce the bandwidth waste.  %which is beneficial 
%If free slots or network resources are not enough, the middlebox chain and application VMs have to be placed separately but in closed racks or sub-trees to reduce the bandwidth consumption caused by communication intra- and inter-tenants. 
...
%we are the first to model both the  virtual middlebox and application network placement problem in multi-tenant datacenter networks with minimal performance guarantees.
% We evaluate the benefits of vMAP through extensive simulations based on a large-scale datacenter topology. %  we show that the effectiveness of our VMs placement algorithm. 
...
% tenant's performance guarantee in the multi-tenant datacenter. 
...
%provide NFV management and orchestration as applications.
 ...
 %In contrast, we first study the placment problem of the virtual middlebox network with intra- and inter-tenant communication 
 %placement problem with bandwidth guarantee
%consumption
...
% Defining a communication model for a middlebox service chain or an intra-tenant VMs' communication is trial, 
\textbf{Approach to accommodating the middlebox scaling request.}
It is a remarkable fact that %the practical application workload is variable. 
...
Therefore, %to better handle the practical workload, 
...
%\section{Virtual Middlebox Network}
...
To provide predictable network performance for the communication of $N$ APP VMs in one tenant, users %typically
...
%Since a tenant may need to apply different middlebox policies to different traffic class, the tenant request can be classified into multiple MB chains associated with different applications \cite{stratos12, M14sap, 7243304}. See Fig. \ref{fig:chain} as an example. The Internet traffic will pass Firewall, Intrusion Detection System and then NAT proxy, while the traffic from other tenants may only pass Firewall and Load Balancer. 
...
%%SDN based traffic engineering manipulates the underlying network routers to explicitly direct the traffic through each middlebox sequentially. If there is no sufficient slot or network resource, the request will be rejected. 
...
\subsection{Overview of the Placement Strategy}%Algorithm}
...
%avoid the generation of extra bandwith consumption .
...
%giving consideration to both beneficial placement principles above at once is not trivial 
when deploying the tenant request due to the limited VM slot resource. %in practice. 
...
%violating the enforced principle before. %previously.
%The violation of the enforced principle is worthy when the achieved benefit outweighs the violation loss.
...
%we have to make tradeoff between the two benefical placement principles above  due to limited VM slot resources in practice. And resultantly, we have our exchanging placement algorithm to better place the middleboxes and application. 

%In this section, we first analyze the bandwidth requirement (bandwidth genaration) for the tenant request. Next, we give a two-step placement strategy according to the bandwidth requirement analysis. Finally, we present our placement algorithm to efficiently deploy the VMN model. 
...
%\begin{comment} 
%We assume each instance of middlebox $k$ ($MB_k$) has
...
%\end{equation}
%where $N\cdot B^{ex}$ is the total incoming traffic. Then we can obtain the total number of VMs needed to be assigned to the middleboxes and applications.
%\end{comment}
%For each VMN model, %we assume that 
...
%required VMs for both MBs and APPs 
as shown in Fig. \ref{fig:bw}. %The incoming traffic will pass the $MB_1$ and $MB_2$ successively and finally arrive at the tenant application. 
The total number of instances required by $MB_k$ is indicated by $M_k$\footnote{We assume each instance of $MB_k$ has a maximal traffic processing capacity $c_k$, so the minimum required number of $MB_k$ is defined as 
	...
	$M_{k}={\left\lceil\frac{N \cdot B^{ex}}{c_{k}}\right\rceil}$,
	%\end{equation}
	where $N\cdot B^{ex}$ is the total incoming traffic.}. And $m_k$ denotes the number of instances placed for $MB_k$  in the left sub-tree. %The number of VMs for the application is $n_{app}$ in the left subtree.%
 Correspondingly, the number of instances for $MB_k$ in the right sub-tree is $M_k-m_k$. The numbers of APP VMs in the left and right sub-trees are denoted by $n_{app}$ and $N-n_{app}$ respectively. %Suppose 
 %the arrival traffic is $N \cdot B^{ex}$ and 
% the middlebox instances prefer to send the traffic to its downstream middlebox instances inside the same subtree.

%We then calculate the bandwidth consumption. Suppose 
%the middlebox instances prefer to send the traffic to its downstream middlebox instances inside the same subtree.
As shown in Fig. \ref{fig:bw}, the incoming traffic will pass the $MB_1$ and $MB_2$ successively and  arrive at the tenant application finally. The traffic flows across and inside the sub-trees are illustrated in thick and fine arrows respectively. %Specifically,
%the middlebox prefer to send 
...
%The traffic from $MB_k$ will be partly sent to the downstream middlebox outside the subtree if 
...
\end{aligned}
\end{equation}
where $b_k$ is the required bandwidth\footnote{For each VN model, %we assume that 
...
%\begin{comment}
%We then obtain key conditions to achieve bandwidth saving through careful placement of MB and APP VMs from Eq. \ref{equ:bw_equation}. 
...
Thus, we consider to achieve bandwidth saving by reducing the bandwidth consumption of the remaining three compositions, i.e., $B_{mb_k,mb_{k+1}}$, $B_{mb,app}$, and $B_{app}$. %which can be reduced by careful placment. %since $B_{switch,mb_1}$ is a constant. 
%\end{comment}
%Based on above results, 
More specifically, to reduce the traffic $B_{mb_k,mb_{k+1}}$ and $B_{mb,app}$ crossing the sub-trees caused by the mismatched capacities between the upstream and downstream MBs or APPs, we can place the MBs and APPs proportionally in the sub-tree according to their traffic processing capacity. In addition, %as $n_{app}$ increases from zero to $N$, 
...
%While the conflict or violation is worthy when the achieved benefit of the principle violation outweighs the violation loss.
...
%by carefully violating the enforced \emph{principle 1}. 
Specifically, to increase the degree of APP co-location in one sub-tree (enforce \emph{principle 2}), we can exchange the positions of MB and APP in two sub-trees when \emph{principle 1} is already enforced. %The position exchanging may result in a change of bandwidth consumption.
...
%The bandwidth requirement of $B_{mb_k,mb_{k+1}}$ and $B_{mb,app}$ will have an increment due to the mismatched capacity in the subtree   
%In the ideal case, the tenant request is fully placed in one subtree and the bandwidth compostions $B_{mb_k,mb_{k+1}}$, $B_{mb,app}$ and $B_{app}$ are reduced to zero. While in the genaral situation, one subtree is unable to accommodate the full tenant request and the VMs for middlebox and application of the request have to be distributed among two or more subtrees.    
%Thus, we get the following two-step placement strategy: 1) placing the MBs and APPs proportionally according to their practical required capacity or numbers requested initially\footnote{Assume the total free VM slots in the left subtree is $N_{slot}$, then the number of $MB_k$ %needed to be 
...
%position exchanging to help to select the proper MB to exchange.% in detail.
\begin{figure}
	\centering
...
%the proportion relation breaking or capacities mismatching, 
...
%of bandwidth reduction by colocating APPs is not able to make up for the cost of increased portion,
which is not recommended in our design. 
%The total bandwidth requirement then have an increase of $C_1-C_2$. 
However, if $C_1<C_2$, there is benefit by exchanging VMs since the total bandwidth cost achieves 
%to exchange since the total bandwidth cost will have 
a reduction of $C_2-C_1$. 
%Therefore, the total bandwidth consumption may have an increase or decrease after the exchange.
...
The result of exchange is shown in Fig. \ref{fig:exchange}(b). The dashed arrows show the change of traffic after exchange. The label associated with each arrow is the value of traffic change. %and the plus and minus signs indicate the increase or decrease for the change respectively.
...
%Generally, the bandwidth requirements in links are various and the. 
We further analyze the bandwidth variation on the single link %to obtain enhanced 
based on the exchanging strategy.   
%we currently only consider the total bandwidth variation of the two links. 
%For a single link, there may be a different result, i.e., the total decrease of the bandwidth cannot ensure that both of the links have a reduction of bandwidth. 
...
C_l<0\quad and \quad C_r<0 %\ i.e. \ B^{ex}+b_2<B^{in} \ and \ b_2<B^{in}.
\end{aligned}
\end{equation}
...
%we cannot achieve a total bandwidth saving but also ensuring a bandwidth reduction for each of the links when placing the tenant request.
...
\begin{algorithm}%[!htbp]
	...
	\begin{algorithmic}[1]
		...
		\If{freeVM(v)$>$0}
		\If{freeVM(v)$<$TotalVM($N_{mb\_app}$)}
		\State place
		% $N_{v\_mb\_app}$
...
\begin{algorithm}[H]%[!htbp]
	...
	\begin{algorithmic}[1]
		\Function{ExchangeMbApp}{t,$N_{mb\_app}$,$N_{v\_mb\_app}$}
		\State mb = GetExchangeableMB(t);
		\If{mb!=null}
		\State $num$ is assigned as the number of types
		\State of the exchangeable MBs;
		\State $k = 0$;
		\If{$N_{mb\_app}[app]>N_{v\_mb\_app}[app]$}
		\While{$k<num \quad and\quad N_{v\_mb\_app}!=0$} 
		\If{$N_{mb\_app}[mb[k]]==0$}
		\State $k$++; $continue$;
		\EndIf
...
%To address the problem of unpredictable MB performance, 
...
%If multiple units of the scaling requirement do not occur simultaneously, 
we can provide few redundancy which provides tradeoff between utilization and availability. By sharing free VMs,
%between variable subrequirements,
a specific MB can be soon deployed on the free VM once the scale-up/out requirement comes. %Although the subrequirement occurs with a probability less than 1, the collision may happen if there is not enough VM provided. To guarantee the performance, we apply the probabilistic model to solve this problem. 
For simplicity, we reuse the notation $T$ to indicate how many types of MBs are required by the tenant. Let $X$ represent the number of middlebox instances actually scaled. % and $X_{i}$ (taking values in \{0, 1\}) indicate whether the $i$-th scaling subrequirement occurs. 
Clearly, $X=\sum_{i=1}^{T}X_{i}\cdot m_{i}$, where $X_{i}$ 
(taking values in \{0, 1\}) 
indicates whether the scaling request of $MB_i$ occurs.
%$i$-th scaling subrequirement occurs. 
...
%is the i-th scaling subrequirement 
...
\end{aligned} .
\end{equation}
%  which is subject to using Chernoff bound, we can obtain a more  tighter result:
%\begin{equation}
%	\begin{aligned} 
%		Pr\left[X>v \right] \leq e^{-\frac{\delta^2\cdot\mu}{3}
%	\end{aligned} 
%\end{equation}
Therefore, the total number of MB VMs to be allocated for one tenant is $|M|+v$, where $|M|$ is the number of basic MBs requirement and $v$ is the minimum additional free VMs to be allocated as backup resource. %Specifically, the reserved free VMs are pre-allocated under the same subtree as normal allocated VMs, which can greatly reduce unnecessary  bandwidth consumption crossing racks or subtrees.
...
%We evaluate our placement algorithm via simulation. 
...
Each PM has 8 slots.
%And every PM is divided into 8 slots. 
The oversubscription ratio is 4:1 at the leaf level and 10:1 at the spine layer.  




%To verify the effectiveness of our probability model based scaling approach, we simulate 



\begin{figure}[h]
	\centering
	\subfigure[Accept rates under different $B^{in}$]{
		\includegraphics[width=3.2in]{fig/accept_rate_bin.pdf}
	...
	%\begin{figure}
	\subfigure[Accept rates under different $B^{ex}$]{
		\centering
		\includegraphics[width=3.2in]{fig/accept_rate_bex.pdf}
...
\textbf{Tenant.} %To simulate the tenant request, we model the 
...
%is equally distributed between 3 to 10.
...
%vMAP can always accept tenant requests reposefully. %And even with a large} $B^{ex}$,}
%\begin{window}[0,c, 
%	includegraphics[]
%\fbox{
%\end{window}
%And even with a large $B^{ex}$, vMAP ensures a high accept rate.
%can keep a relatively high accept rate. 
...
\begin{figure}[h]
	\centering	
	\subfigure[VM slot utilizations under different $B^{in}$]{
		\label{fig:vm_util_bin}
		\includegraphics[width=3.2in]{fig/vm_util_bin.pdf}
	}
	\subfigure[VM slot utilizations under different $B^{ex}$]{
		\label{fig:vm_util_bex}
		\includegraphics[width=3.2in]{fig/vm_util_bex.pdf}
	}
	%\label{fig:vm_util}
...
%the number of total VMs required for tenant requests increases as $B^{ex}$ increases since we need more middlebox instances to process a larger external traffic. This leads to 
the increased demand for VM slots in the datacenter, the VMs of the tenant will be spread across more racks.
%and the tenant requests are forced to spread their VMs across more racks. 
...
%Thus, vMAP is helpful to allow the provider to accommodate more tenant requests while reduce the bandwidth consumption of core-level links
By doing so, the proposed strategy vMAP allows more tenant requests to be accepted. %which allows more future tenant requests to be accepted.
%\end{window}

\begin{figure}[h]
	\centering
	\subfigure[Core-aggregation links' bandwidth utilizations]{
		\label{fig:agg_core}
		\includegraphics[width=3.2in]{fig/high_bw_util.pdf}}
	\subfigure[ToR-PM links' bandwidth utilizations]{
		\label{fig:pm_agg}
		\includegraphics[width=3.2in]{fig/low_bw_util.pdf}}
...
This is reasonable since higher accept rates represent that
 %there are more requests accepted and 
...
%servers are on the same layer of the three-level network topology, if there is one download bandwidth consumption, there must be another upload bandwidth consumption correspondingly. 
...
%can be controlled under the threshold stablely by reserving a few free VMs for the scaling requirement, while the collision rates are comparatively high if no free VMs are reserved. 
\begin{figure}%[H]
	\centering 
...
%And a large collision threshold is helpful for a high reserved VMs' utilization.
%that a higher reserved VMs'utilization relates to a larger collision threshold. 
...
% we demonstrate the simulation results of the correlations in Fig. \ref{fig:vm_util_p}.
%As shown by the two lines,
...
%when collision threshold $\rho=0.4$. 
...
Since middleboxes are widely used in current networks, it is essential to consider middleboxes when deploying the tenant request with performance guarantee. %embedding predicable virtual networks. 
 %This paper tries to fill the gap. 
%To achieve this goal, 
In this paper, we introduced vMAP, a joint virtual middlebox and application deployment solution. We first model the tenant's requirement of software middleboxes and applications by augumenting software middleboxes as VMs in the Tenant Application Graph. A VM placement algorithm %called EMA, 
...
%solution for scalibility issues.
%requirements.
...
%while the probability model based middlebox scaling approach can accommodate the variable requirements effectively.   
...
He currently focuses on the field of resource management in datacenter networks.
\end{window} 
%\begin{wrapfigure}{l}{25mm}
\begin{window}[0,l,
	{\includegraphics[width=1.0in]{zhangxw.png}},]
%\end{wrapfigure} 
\textbf{Xuewei Zhang} received her Bachelor degree and Master degree from the Department of Computer Science and Technology of Nanjing University in 2014 and 2017, respectively. Her research interests include datacenter network system and scheduling algorithm.
\end{window}
\vspace{2em}
\begin{window}[0,l,
...
%\newpage
\begin{window}[0,l,
	\includegraphics[width=1.0in]{lu.png},]
\textbf{Sanglu Lu} 
received the BS, MS, and PhD
degrees from Nanjing University in 1992, 1995,
and 1997, respectively, all in computer science.
She is currently a professor in the Department of
Computer Science and Technology and the State
Key Laboratory for Novel Software Technology.
Her research interests include distributed computing, wireless networks, and pervasive computing. She has published more than 80 papers
in referred journals and conferences in the above
areas. She is a member of the IEEE.
\end{window} 
%\end{section}
% that's all folks
\end{document}
